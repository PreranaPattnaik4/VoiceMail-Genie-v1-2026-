...
import { initAuth, googleSignIn, logout, getTokenStatus, isTokenExpired, enterSandboxMode, signUpWithEmailPassword, loginWithEmailPassword, sendPasswordReset } from "./firebase-auth";
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'voice' | 'template' | 'compose' | 'library'>('voice');
      ...
      if (saved) {
        try {
          ...
          return parsed.filter(d => 
            !d.subject?.toLowerCase().includes('pattnaiksujit.me') && 
            !d.body?.toLowerCase().includes('pattnaiksujit.me')
          );
        } catch {}
      }
    }
    ...
    if (typeof window !== 'undefined') {
      ...
      if (saved) {
        try {
          ...
          return parsed.filter(email => email !== 'pattnaiksujit.me@gmail.com');
        } catch {}
      }
    }
    return [
      "sarah.jones@acme.com",
      "support@voicemailgenie.com"
    ];
  });
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      let errorMsg = err.message || "Authentication failed. Please check popup blockers.";
      if (isIframe || errorMsg.includes("popup") || errorMsg.includes("closed") || errorMsg.includes("storage") || errorMsg.includes("network")) {
        errorMsg += " Tip: Popups/cookies may be blocked in the iframe. Please click the 'Open in a new tab' icon in the top-right of the preview pane to authorize Google successfully. Alternatively, you can bypass this with Sandbox Mode.";
      }
      setDraftStatusMessage(errorMsg);
    } finally {
  ...
  const handleSandboxLogin = () => {
    ...
    setDraftStatus('idle');
    try {
      const result = enterSandboxMode();
      setUser(result.user);
      setAccessToken(result.accessToken);
      setAuthStatus('connected');
      setNeedsAuth(false);
      setDraftStatus('success');
      setDraftStatusMessage("Sandbox Mode activated! Explore all features freely.");
      showToast("Sandbox Demo Mode activated!", 'success');
      setTimeout(() => {
        setDraftStatus('idle');
        setDraftStatusMessage("");
      }, 4000);
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    setIsLoggingIn(true);
    try {
      const newUser = await signUpWithEmailPassword(regName.trim(), regEmail.trim(), regPassword);
      setUser(newUser);
      setAccessToken(null);
      setNeedsAuth(true);
      setAuthSuccess("Account created successfully! Welcome to VoiceMail Genie.");
      showToast("Account created successfully!", "success");
    } catch (err: any) {
      console.error("Sign up failed:", err);
      if (err.code === "auth/operation-not-allowed") {
        setAuthError("Email/Password sign-in is disabled in your Firebase Console. To enable it: 1) Go to your Firebase Console, 2) Navigate to Build -> Authentication -> Sign-in method, 3) Click 'Add new provider', select 'Email/Password' and Enable it. Alternatively, please Sign in with Google below!");
      } else if (err.code === "auth/email-already-in-use") {
        setAuthError("This email is already in use by another account.");
      } else if (err.code === "auth/invalid-email") {
        setAuthError("Please enter a valid email address.");
      } else if (err.code === "auth/weak-password") {
        setAuthError("The password is too weak. Please choose a stronger password.");
      } else {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    setIsLoggingIn(true);
    try {
      const loggedUser = await loginWithEmailPassword(loginEmail.trim(), loginPassword, loginRememberMe);
      setUser(loggedUser);
      setAccessToken(null);
      setNeedsAuth(true);
      setAuthSuccess("Logged in successfully! Welcome back.");
      ...
      if (err.code === "auth/operation-not-allowed") {
        ...
        setAuthError("Too many failed attempts. Access temporarily locked. Reset password or try later.");
      } else {
  ...
  const saveToGmailDraft = async () => {
    ...
    // If they have no Google token but are a logged-in user, we use sandbox simulation
    if (currentStatus.status === 'disconnected' && user) {
      activeToken = "sandbox_token";
    } else if (currentStatus.status === 'disconnected') {
      setDraftStatus('error');
    ...
    if (activeToken !== "sandbox_token" && (currentStatus.status === 'expired' || isTokenExpired())) {
    ...
    if (activeToken === "sandbox_token") {
      ...
      try {
        ...
        // Add to local savedDrafts
        const newDraft: SavedDraft = {
          id: `draft-${Date.now()}`,
          subject: manuallyEditedSubject,
          body: manuallyEditedBody,
          category: "Client Emails", // default
        ...
        if (typeof window !== 'undefined') {
          localStorage.setItem('saved_drafts', JSON.stringify(updatedDrafts));
        }
        
        setDraftStatus('success');
        setDraftStatusMessage("Sandbox Mode: Simulated success! Saved copy to 'My Library'.");
        showToast("Draft simulated and added to My Library!", 'success');
        
        // Save recipient to recentRecipients list
        if (recipientEmail && recipientEmail.trim().includes('@')) {
          const trimmedEmail = recipientEmail.trim();
          if (trimmedEmail !== 'pattnaiksujit.me@gmail.com' && !recentRecipients.includes(trimmedEmail)) {
            const updated = [trimmedEmail, ...recentRecipients].slice(0, 10);
  ...
  const addCategory = (name: string) => {
    ...
    const updated = [...customCategories, name.trim()];
  ...
  const renameCategory = (oldName: string, newName: string) => {
    ...
    const updatedCats = customCategories.map(c => c === oldName ? newName.trim() : c);
    setCustomCategories(updatedCats);
    if (typeof window !== 'undefined') {
      localStorage.setItem('custom_categories', JSON.stringify(updatedCats));
    }
    const updatedDrafts = savedDrafts.map(d => d.category === oldName ? { ...d, category: newName.trim() } : d);
    setSavedDrafts(updatedDrafts);
    if (typeof window !== 'undefined') {
      localStorage.setItem('saved_drafts', JSON.stringify(updatedDrafts));
    }
  ...
  const deleteCategory = (categoryName: string) => {
    if (confirm(`Are you sure you want to delete the folder "${categoryName}"? Saved emails in this folder will be uncategorized.`)) {
      ...
      const updatedDrafts = savedDrafts.map(d => d.category === categoryName ? { ...d, category: "Uncategorized" } : d);
      setSavedDrafts(updatedDrafts);
      if (typeof window !== 'undefined') {
        localStorage.setItem('saved_drafts', JSON.stringify(updatedDrafts));
      }
  ...
  const toggleFavoriteDraft = (id: string) => {
    const updatedDrafts = savedDrafts.map(d => d.id === id ? { ...d, isFavorite: !d.isFavorite } : d);
    setSavedDrafts(updatedDrafts);
    if (typeof window !== 'undefined') {
      localStorage.setItem('saved_drafts', JSON.stringify(updatedDrafts));
    }
  ...
  const deleteRecipient = (emailToDelete: string) => {
    const updated = recentRecipients.filter(email => email !== emailToDelete);
    setRecentRecipients(updated);
    if (typeof window !== 'undefined') {
      localStorage.setItem('recent_recipients', JSON.stringify(updated));
    }
    if (recipientEmail === emailToDelete) {
      setRecipientEmail("");
    }
    showToast(`Deleted "${emailToDelete}" from contacts!`, 'info');
  };
  ...
  const isAppAuthenticated = !!user || (typeof window !== 'undefined' && localStorage.getItem('is_sandbox_mode') === 'true');

  if (isAuthLoading) {
    return (
      <div className="min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-center p-4">
        <div className="flex flex-col items-center gap-4">
          <div className="w-12 h-12 bg-indigo-600 rounded-2xl flex items-center justify-center shadow-lg shadow-indigo-100 animate-pulse">
            <Sparkles className="w-6 h-6 text-white" />
          </div>
          <div className="flex items-center gap-2">
            <Loader2 className="w-4 h-4 animate-spin text-indigo-600" />
            <span className="text-sm font-semibold text-slate-600">Initializing Genie...</span>
          </div>
        </div>
      </div>
    );
          ...
          <button
            ...
            className={`p-2 rounded-xl transition-all cursor-pointer flex items-center justify-center border ${
              isSidebarOpen 
                ...
                : "bg-white border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-indigo-600 hover:border-indigo-100"
            }`}
          ...
          <div className="flex items-center gap-3">
            {!accessToken && (
              <button
                onClick={handleLogin}
                disabled={isLoggingIn}
                className="relative flex items-center gap-1.5 px-3 py-1.5 border border-slate-200 hover:border-indigo-200 rounded-xl bg-white text-slate-600 hover:text-indigo-600 font-bold text-xs shadow-xs hover:bg-slate-50 transition-all cursor-pointer"
                title="Link your real Gmail account to directly sync generated email drafts"
              >
                {isLoggingIn ? (
            ...
            <div className="flex items-center gap-2 pl-2 pr-1.5 py-1 rounded-xl border bg-slate-50 border-slate-200/80 text-slate-700">
              <div className="flex items-center gap-1.5">
                {user?.photoURL ? (
                  ...
                  <div className="h-6 w-6 rounded-full bg-indigo-600 text-white flex items-center justify-center text-[10px] font-extrabold uppercase">
                ...
                <div className="flex flex-col text-left max-w-[100px]">
                  <span className="hidden md:inline text-[11px] font-extrabold truncate leading-tight">
                    {user?.displayName || (user?.email ? user.email.split("@")[0] : "Genie User")}
                  </span>
                  <span className="hidden md:inline text-[8px] font-semibold text-slate-400 uppercase leading-none mt-0.5 tracking-wide">
                    {accessToken ? "Gmail Sync" : "Sandbox Mode"}
                  </span>
                </div>
                {accessToken && (
                  <span className="bg-green-100 border border-green-200/50 text-green-800 text-[8px] font-extrabold px-1 py-0.5 rounded uppercase tracking-wider">Linked</span>
                )}
              </div>
              <button
                onClick={handleLogout}
                className="p-1 rounded-lg transition-all cursor-pointer text-slate-500 hover:text-rose-600 hover:bg-slate-100"
                title="Sign Out"
              >
        ...
        <div className="flex-grow flex flex-col min-w-0 bg-[#F1F5F9]">
          ...
          <div className="bg-white border-b border-slate-200 px-6 py-3 flex flex-wrap items-center justify-between gap-4 sticky top-0 z-10">
            ...
            <div className="text-[10px] font-bold text-slate-400 flex items-center gap-1.5 uppercase tracking-wide">
              ...
              {activeTab === 'library' && "Organize and review your saved drafts in custom folders"}
            </div>
          </div>

          {/* Main Content View (Bento Grid or Separate My Library Page) */}
          {activeTab === 'library' ? (
            <div className="flex-grow bg-slate-50/50 overflow-y-auto max-h-[calc(100vh-7.5rem)]">
              <div className="max-w-7xl mx-auto p-4 md:p-6 lg:p-8 space-y-6">
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
                  
                  {/* Left Column: Folders Sidebar */}
                  <div className="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-xs space-y-5 lg:sticky lg:top-6">
                    <div className="flex items-center justify-between">
                      <span className="text-xs uppercase tracking-widest font-extrabold text-slate-400">Library Folders</span>
                      <button
                        onClick={() => setIsAddingCategory(true)}
                        className="px-2.5 py-1 rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-700 hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer flex items-center gap-1 text-[11px] font-bold border border-indigo-100"
                      >
                    ...
                    {isAddingCategory && (
                      <div className="flex items-center gap-1.5 bg-slate-50 p-2 rounded-xl border border-slate-200">
                        <input
                          ...
                          className="flex-grow text-xs p-1.5 bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-500 font-semibold"
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') addCategory(newCategoryName);
                            if (e.key === 'Escape') setIsAddingCategory(false);
                          }}
                        ...
                        <button
                          onClick={() => addCategory(newCategoryName)}
                          className="p-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg cursor-pointer flex items-center justify-center"
                        >
                          <Check className="w-3.5 h-3.5" />
                        </button>
                        <button
                          onClick={() => setIsAddingCategory(false)}
                          className="p-1.5 bg-slate-200 hover:bg-slate-300 text-slate-600 rounded-lg cursor-pointer flex items-center justify-center"
                        >
                          <X className="w-3.5 h-3.5" />
                        </button>
                      </div>
                    ...
                    <div className="space-y-1.5 max-h-[400px] overflow-y-auto pr-1 scrollbar-thin">
                      <button
                        onClick={() => setSelectedLibraryCategory("All")}
                        className={`w-full flex items-center justify-between p-2.5 rounded-xl text-left text-xs font-bold transition-all border cursor-pointer ${
                          selectedLibraryCategory === "All"
                      ...
                      {customCategories.map(cat => {
                        ...
                        return (
                          <div
                            key={cat}
                            className={`group flex items-center justify-between p-1 rounded-xl text-xs transition-all border ${
                              isSelected
                          ...
                          >
                            {isEditingThis ? (
                              <div className="flex-grow flex items-center gap-1.5 px-1.5 py-0.5 w-full">
                                <input
                                  type="text"
                                  value={editingCategory?.newName || ""}
                                  onChange={(e) => setEditingCategory({ ...editingCategory!, newName: e.target.value })}
                                  className="flex-grow text-xs p-1 bg-white border border-slate-200 rounded-lg focus:outline-none font-semibold"
                                  onKeyDown={(e) => {
                                    if (e.key === 'Enter') renameCategory(cat, editingCategory!.newName);
                                    if (e.key === 'Escape') setEditingCategory(null);
                                  }}
                              ...
                              <>
                                <button
                                  ...
                                  className={`flex-grow flex items-center gap-2 p-1.5 text-left font-bold transition-all cursor-pointer ${
                                ...
                                <div className="flex items-center gap-1 pr-1.5 shrink-0">
                                  <span className="px-2 py-0.5 bg-slate-100 text-slate-500 text-[9px] rounded-full font-bold mr-1 group-hover:hidden">
                                    {count}
                                  </span>
                                  <button
                                    onClick={(e) => {
                                    ...
                                    className="p-1 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded opacity-0 group-hover:opacity-100 transition-all cursor-pointer"
                                    title="Rename folder"
                                  >
                  ...
                  {/* Right Column: Search & Saved Drafts list */}
                  <div className="lg:col-span-3 space-y-6">
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
                      <div className="space-y-1">
                        <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                          <span>{selectedLibraryCategory === "All" ? "📂 All Saved Drafts" : `📁 ${selectedLibraryCategory}`}</span>
                        </h2>
                        <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">
                          Reviewing {
                            savedDrafts.filter(draft => {
                              if (selectedLibraryCategory !== "All" && draft.category !== selectedLibraryCategory) return false;
                              if (showFavoritesOnly && !draft.isFavorite) return false;
                              if (librarySearchQuery) {
                                const q = librarySearchQuery.toLowerCase();
                                return draft.subject?.toLowerCase().includes(q) || draft.body?.toLowerCase().includes(q);
                              }
                              return true;
                            }).length
                          } matching drafts
                      ...
                      <div className="flex items-center gap-2.5 flex-wrap">
                        ...
                        <div className="relative min-w-[200px] flex-grow md:flex-grow-0">
                          ...
                          <input
                            ...
                            className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-500 transition-all placeholder-slate-400"
                          />
                          {librarySearchQuery && (
                            <button 
                              onClick={() => setLibrarySearchQuery("")}
                              className="absolute right-2 top-2.5 p-0.5 rounded-full hover:bg-slate-200 text-slate-400 cursor-pointer flex items-center justify-center"
                            >
                              <X className="w-3 h-3" />
                            </button>
                          )}
                        ...
                        >
                          <Heart className={`w-3.5 h-3.5 ${showFavoritesOnly ? "fill-amber-500 text-amber-500" : ""}`} />
                          <span>Favorites</span>
                        </button>
                      </div>
                    </div>
                    ...
                    {/* Drafts Grid */}
                    {(() => {
                      const displayed = savedDrafts.filter(draft => {
                        if (selectedLibraryCategory !== "All" && draft.category !== selectedLibraryCategory) return false;
                        if (showFavoritesOnly && !draft.isFavorite) return false;
                        if (librarySearchQuery) {
                          const q = librarySearchQuery.toLowerCase();
                      ...
                      if (displayed.length === 0) {
                        return (
                          <div className="bg-white rounded-2xl border border-slate-200/80 p-12 text-center space-y-4 shadow-xs">
                            <div className="w-14 h-14 bg-indigo-50 rounded-full flex items-center justify-center text-indigo-500 mx-auto">
                              <Bookmark className="w-6 h-6 text-indigo-600" />
                            </div>
                            <div className="space-y-1">
                              <h3 className="text-base font-bold text-slate-800">No matching saved drafts</h3>
                              <p className="text-xs text-slate-400 max-w-md mx-auto leading-relaxed font-semibold">
                                {librarySearchQuery || showFavoritesOnly 
                                  ? "Try clearing your filters or search terms to view all saved emails."
                                  : "Click 'Save to Library' inside the Review Workspace to store drafts here."}
                              </p>
                            </div>
                            {(librarySearchQuery || showFavoritesOnly || selectedLibraryCategory !== "All") && (
                              <button
                                onClick={() => {
                      ...
                      return (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {displayed.map(draft => (
                            <div
                              key={draft.id}
                              className="bg-white border border-slate-200/80 hover:border-indigo-200 hover:shadow-xs rounded-2xl p-5 flex flex-col justify-between transition-all group relative"
                            >
                              <div className="space-y-3">
                                <div className="flex items-start justify-between gap-3">
                                  ...
                                  <div className="flex items-center gap-1.5 shrink-0">
                                    ...
                                    <button
                                      ...
                                      title={draft.isFavorite ? "Remove Favorite" : "Favorite"}
                                    >
                                      <Heart className={`w-4 h-4 ${draft.isFavorite ? "fill-amber-500 text-amber-500" : ""}`} />
                                    </button>
                                ...
                                <div className="space-y-1">
                                  ...
                                  <p className="text-[10px] text-slate-400 font-bold">{draft.savedAt}</p>
                                </div>

                                {/* Strip tags for readable library preview */}
                                <div className="bg-slate-50 border border-slate-100/50 rounded-xl p-3 max-h-[140px] overflow-y-auto text-xs text-slate-600 leading-relaxed scrollbar-thin">
                                  {draft.body.replace(/<[^>]*>/g, '').substring(0, 350)}
                                  {draft.body.replace(/<[^>]*>/g, '').length > 350 && "..."}
                                </div>
                              </div>
                              ...
                              <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-end">
                                <button
                                  onClick={() => {
                                    ...
                                    setManuallyEditedSubject(draft.subject);
                                    setManuallyEditedBody(draft.body);
                                    setActiveTab('voice'); // Redirect to main voice mode draft reviews
                                    showToast("Loaded saved draft into editor workspace!", 'success');
                                  }}
                                  className="w-full py-2 bg-slate-900 hover:bg-indigo-600 text-white font-extrabold text-[11px] rounded-xl tracking-wide flex items-center justify-center gap-1.5 cursor-pointer hover:scale-[1.01] active:scale-[0.99] transition-all"
                                >
                                  <span>Load in Editor Workspace</span>
                ...
                {authStatus !== 'connected' ? (
                  <div className="space-y-2">
                    ...
                    {typeof window !== 'undefined' && window.self !== window.top && (
                      <div className="p-2.5 bg-indigo-50/70 border border-indigo-100 rounded-xl text-[10px] text-indigo-700 flex items-start gap-1.5 leading-tight">
                        <span className="text-xs shrink-0">💡</span>
                        <span>
                          <strong>Preview Mode Notice:</strong> Browser security rules in the preview pane can block Google Auth popups. If 'Connect Gmail' fails, click the <strong>'Open in a new tab'</strong> icon (top right of the preview) to connect instantly.
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-2.5">
                    <div className="flex flex-col sm:flex-row gap-2 items-end">
                      <div className="flex-grow space-y-1 w-full">
                        ...
                        <input
                          ...
                          className="w-full text-xs p-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-500 placeholder-slate-400 font-mono"
                        />
                      </div>
                    ...
                    {/* Recent Recipients list with Delete Buttons */}
                    {recentRecipients.length > 0 && (
                      <div className="bg-slate-50 border border-slate-200/65 rounded-xl p-2.5">
                        <div className="flex items-center justify-between mb-1.5">
                          <span className="text-[9px] font-bold text-slate-500 uppercase tracking-wider">Quick Contacts / Saved IDs</span>
                          <span className="text-[8px] text-slate-400 font-medium">Click to select • Click ✕ to delete</span>
                        </div>
                        <div className="flex flex-wrap gap-1.5 max-h-[100px] overflow-y-auto pr-1">
                          {recentRecipients.map((email) => (
                            <div 
                              key={email}
                              className={`group flex items-center gap-1 text-[10px] pl-2 pr-1 py-1 rounded-lg border transition-all ${
                                recipientEmail === email 
                                  ? 'bg-indigo-50 border-indigo-200 text-indigo-700 font-medium shadow-xs' 
                            ...
                            >
                              <button
                                ...
                                className="truncate hover:underline cursor-pointer font-mono"
                              >
                                {email}
                              </button>
                              <button
                                ...
                                className="p-0.5 rounded text-slate-400 hover:text-rose-600 hover:bg-rose-50 transition-all cursor-pointer opacity-80 group-hover:opacity-100"
